...
describe("transfer-display", () => {
  ...
  it("classifies anonymous groups (empty or job/N)", () => {
    ...
    expect(isAnonymousGroup("job/42")).toBe(false)
    expect(isAnonymousGroup("job/9999")).toBe(false)
  ...
  it("rolls up past transfers by named group, with anonymous items in their own bucket", () => {
    ...
    // groups sort by latest completedAt desc; items without group stay at end
    ...
    const photos = groups.find((g) => g.key === "group:photos-batch")!
    expect(photos.label).toBe("photos-batch")
    expect(photos.isSyntheticJobGroup).toBe(false)
    ...
    const job = groups.find((g) => g.key === "group:job/42")!
    expect(job.label).toBe("Job 42")
    expect(job.isSyntheticJobGroup).toBe(true)
    expect(job.items).toHaveLength(1)
    expect(job.totalBytes).toBe(10)
  })

  it("uses effective bytes for completed server-side copies in past group totals", () => {
    const [group] = buildPastGroupModels([
      {
        name: "videos/movie.mkv",
        group: "server-side-batch",
        bytes: 0,
        size: 2_366_920_044,
    ...
    expect(group?.key).toBe("group:server-side-batch")
    expect(group?.totalBytes).toBe(2_366_920_044)
    expect(group?.totalSize).toBe(2_366_920_044)
  })
