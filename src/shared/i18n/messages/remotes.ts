...
const remotesMessages: MessageSet<AppMessages["remotes"]> = {
  en: {
    ...
    invalidRemoteNameEntries: (names, hasMore) => `Invalid remote names: ${names}${hasMore ? "..." : ""}`,
    missingTypeEntries: (names, hasMore) => `Missing or invalid type: ${names}${hasMore ? "..." : ""}`,
  ...
  "zh-CN": {
    ...
    invalidRemoteNameEntries: (names, hasMore) => `存储名不合法：${names}${hasMore ? "..." : ""}`,
    missingTypeEntries: (names, hasMore) => `缺少或无效的 type：${names}${hasMore ? "..." : ""}`,
    ...
    invalidRemoteName: () =>
      "存储名包含非法字符。rclone 只允许字母、数字、`_`、`-`、`.`、`+`、`@`，以及位于各段之间的空格；但名称不能以 `-` 或空格开头，也不能以空格结尾。",
